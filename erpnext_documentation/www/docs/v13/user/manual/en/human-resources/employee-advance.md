...
After saving Employee Advance, Employee should [Assign document to Approver](/docs/v13/user/manual/en/using-erpnext/assignment). On assignment, approving user will also receive email notification. To automate email notification, you can also setup [Email Alert](/docs/v13/user/manual/en/setting-up/notifications.html).
...
After submission of Employee Advance record, accounts user will be able to create a [Payment Entry](/docs/v13/user/manual/en/accounts/payment-entry) using the 'Create' button.
...
Later when the employee claims the expense, an advance record can be fetched in the [Expense Claim](/docs/v13/user/manual/en/human-resources/expense-claim) and linked to the claim record.
...
* If amount is unused, click on the **Return** button to return the paid Advance amount
    ![Return Button](/docs/v13/assets/img/human-resources/advance-return-button.png)
...
* If only some amount is claimed and rest is returned, the returned amount will be shown in the 'Returned Amount' field.
    ![Return advance Amount](/docs/v13/assets/img/human-resources/advance-returned-amount.png)
