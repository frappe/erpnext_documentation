<!-- add-breadcrumbs -->
# Human Resources

The Human Resources (HR) module covers the processes related to the HR department of a company. It maintains a complete employee database including contact information,
salary details, attendance, performance evaluation, leaves, and appraisal records.

The most important feature here is processing the payroll by using Payroll Entry to generate Salary Slips. Most countries have complex tax rules stating which expenses the company can make on behalf of its Employees.

There are a set of rules for the company to deduct taxes and social security
from employee payroll. ERPNext accommodates all types of taxes and
their calculation.

## 1. Topics
### 1.1 Employee
1. [Employee](/docs/v13/user/manual/en/human-resources/employee)
1. [Employment Type](/docs/v13/user/manual/en/human-resources/employment-type)
1. [Branch](/docs/v13/user/manual/en/human-resources/branch)
1. [Department](/docs/v13/user/manual/en/human-resources/department)
1. [Designation](/docs/v13/user/manual/en/human-resources/designation)
1. [Employee Grade](/docs/v13/user/manual/en/human-resources/employee-grade)
1. [Employee Group](/docs/v13/user/manual/en/human-resources/employee-group)
1. [Employee Health Insurance](/docs/v13/user/manual/en/human-resources/health-insurance)

### 1.2 Attendance

1. [Attendance](/docs/v13/user/manual/en/human-resources/attendance)
1. [Employee Attendance Tool](/docs/v13/user/manual/en/human-resources/employee-attendance-tool)
1. [Attendance Request](/docs/v13/user/manual/en/human-resources/attendance-request)
1. [Upload Attendance](/docs/v13/user/manual/en/human-resources/upload-attendance)
1. [Auto Attendance](/docs/v13/user/manual/en/human-resources/auto-attendance)

### 1.3 Leaves
1. [Leave Management](/docs/v13/user/manual/en/human-resources/leave-management-intro/)
1. [Holiday List](/docs/v13/user/manual/en/human-resources/holiday-list)
1. [Leave Type](/docs/v13/user/manual/en/human-resources/leave-type)
1. [Leave Period](/docs/v13/user/manual/en/human-resources/leave-period)
1. [Leave Policy](/docs/v13/user/manual/en/human-resources/leave-policy)
1. [Leave Policy Assignment](/docs/v13/user/manual/en/human-resources/leave-policy-assignment)
1. [Leave Allocation](/docs/v13/user/manual/en/human-resources/leave-allocation)
1. [Leave Application](/docs/v13/user/manual/en/human-resources/leave-application)
1. [Compensatory Leave Request](/docs/v13/user/manual/en/human-resources/compensatory-leave-request)
1. [Leave Encashment](/docs/v13/user/manual/en/human-resources/leave-encashment)
1. [Leave Block List](/docs/v13/user/manual/en/human-resources/leave-block-list)


### 1.4 Recruitment
1. [Staffing Plan](/docs/v13/user/manual/en/human-resources/staffing-plan)
1. [Job Opening](/docs/v13/user/manual/en/human-resources/job-opening)
1. [Employee Referral](/docs/v13/user/manual/en/human-resources/employee-referral)
1. [Job Applicant](/docs/v13/user/manual/en/human-resources/job-applicant)
1. [Job Offer](/docs/v13/user/manual/en/human-resources/job-offer)
1. [Appointment Letter](/docs/v13/user/manual/en/human-resources/appointment-letter)

### 1.5 Training
1. [Training Program](/docs/v13/user/manual/en/human-resources/training-program)
1. [Training Event](/docs/v13/user/manual/en/human-resources/training-event)
1. [Training Result](/docs/v13/user/manual/en/human-resources/training-result)
1. [Training Feedback](/docs/v13/user/manual/en/human-resources/training-feedback)


### 1.6 Employee Lifecycle
1. [Employee Lifecycle Management Introduction](/docs/v13/user/manual/en/human-resources/employee-lifecycle-intro)
1. [Employee Onboarding](/docs/v13/user/manual/en/human-resources/employee-onboarding)
1. [Employee Promotion](/docs/v13/user/manual/en/human-resources/employee_promotion)
1. [Employee Separation](/docs/v13/user/manual/en/human-resources/employee-separation)
1. [Employee Transfer](/docs/v13/user/manual/en/human-resources/employee_transfer)
1. [Employee Skill Map](/docs/v13/user/manual/en/human-resources/employee_skill_map)


### 1.7 Payroll
1. [Payroll Management](/docs/v13/user/manual/en/human-resources/payroll-intro)
1. [Payroll Period](/docs/v13/user/manual/en/human-resources/payroll-period)
1. [Income Tax Slab](/docs/v13/user/manual/en/human-resources/income-tax-slab)
1. [Salary Component](/docs/v13/user/manual/en/human-resources/salary-component)
1. [Salary Structure](/docs/v13/user/manual/en/human-resources/salary-structure)
1. [Salary Structure Assignment](/docs/v13/user/manual/en/human-resources/salary-structure-assignment)
1. [Salary Slip](/docs/v13/user/manual/en/human-resources/salary-slip)
1. [Payroll Entry](/docs/v13/user/manual/en/human-resources/payroll-entry)
1. [Additional Salary](/docs/v13/user/manual/en/human-resources/additional-salary)
1. [Retention Bonus](/docs/v13/user/manual/en/human-resources/retention-bonus)
1. [Employee Incentive](/docs/v13/user/manual/en/human-resources/employee-incentive)



### 1.8 Employee Tax and Benefits
1. [Employee Tax Exemption Declaration](/docs/v13/user/manual/en/human-resources/employee-tax-exemption-declaration)
1. [Employee Tax Exemption Proof Submission](/docs/v13/user/manual/en/human-resources/employee-tax-exemption-proof-submission)
1. [Employee Other Income](/docs/v13/user/manual/en/human-resources/employee-other-income)
1. [Employee Benefit Application](/docs/v13/user/manual/en/human-resources/employee-benefit-application)
1. [Employee Benefit Claim](/docs/v13/user/manual/en/human-resources/employee-benefit-claim)


### 1.9 Performance
1. [Appraisal](/docs/v13/user/manual/en/human-resources/appraisal)
1. [Energy Point System](/docs/v13/user/manual/en/setting-up/energy-point-system)


### 1.10 Expense Claims
1. [Expense Claim](/docs/v13/user/manual/en/human-resources/gratuity)
1. [Employee Advance](/docs/v13/user/manual/en/human-resources/employee-advance)

## 1.11 Gratuity
1. [Gratuity](/docs/v13/user/manual/en/human-resources/gratuity)
1. [Gratuity Rule](/docs/v13/user/manual/en/human-resources/gratuity-rule)

### 1.11 Loans
1. [Loan Management](/docs/v13/user/manual/en/loan-management)
1. [Loan Type](/docs/v13/user/manual/en/loan-management/loan-type)
1. [Loan Application](/docs/v13/user/manual/en/loan-management/loan-application)
1. [Loan](/docs/v13/user/manual/en/loan-management/loan)

### 1.12 Shift Management
1. [Shift Management](/docs/v13/user/manual/en/human-resources/shift-management)

### 1.13 Fleet Management
1. [Fleet Management](/docs/v13/user/manual/en/human-resources/fleet-management)
1. [Vehicle](/docs/v13/user/manual/en/human-resources/vehicle)
1. [Vehicle Log](/docs/v13/user/manual/en/human-resources/vehicle-log)


### 1.14 Settings
1. [HR Settings](/docs/v13/user/manual/en/human-resources/hr-settings)
1. [Daily Work Summary Group](/docs/v13/user/manual/en/human-resources/daily-work-summary-group)


### 1.15 Reports
1. [Human Resources Reports](/docs/v13/user/manual/en/human-resources/human-resources-reports)

## 2. Articles
1. [Leave Calculation In Salary Slip](/docs/v13/user/manual/en/human-resources/articles/leave-calculation-in-salary-slip)
1. [Working Days In Salary Slip](/docs/v13/user/manual/en/human-resources/articles/working-days-in-salary-slip)

## 3. Videos
1. [Employees](/docs/v13/user/videos/learn/employee)
1. [Leave Management](/docs/v13/user/videos/learn/leave-management)
1. [Expense Claims](/docs/v13/user/videos/learn/expense-claim)
1. [Employee Advance](/docs/v13/user/videos/learn/employee-advance)
