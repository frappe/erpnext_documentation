...
For each quiz attempt, till the student is allowed to attempt the quiz, a quiz activity is created. This doctype has all the information about the attempt viz. The selected options for each question, attempt date, as well as the result of the quiz and the time taken to complete, if the quiz is time-bound.
