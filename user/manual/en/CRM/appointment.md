...
Appointments are automatically assigned to employees as per the `Agents` list in [Appointment Booking Settings](docs/user/manual/en/CRM/appointment_booking_settings). The agent with the least assignment for the day of the appointment and who is free in the scheduled time is assigned to the appointment.
